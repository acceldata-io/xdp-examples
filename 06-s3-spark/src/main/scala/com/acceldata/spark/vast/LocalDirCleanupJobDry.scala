  ...
  def main(args: Array[String]): Unit = {
    ...
    if (args.length < 2) {
      throw new RuntimeException(
        """Usage: LocalDirCleanupJobDry <path1,path2,...> <days>
          |
          |Arguments:
          ...
          |
          |Examples:
          |  LocalDirCleanupJobDry /data0/spark-tmp,/data1/spark-tmp 1
          |  LocalDirCleanupJobDry /data0/spark-tmp 0
          |""".stripMargin)
    }
    ...
    val spark = SparkSession.builder()
      .appName("SparkLocalDirCleanupJobDry")
      .getOrCreate()
    ...
    println(s"=== Dry Run Configuration ===")
      ...
      targetDirs.foreach { path =>
        ...
        if (dir.exists() && dir.isDirectory) {
          ...
          if (items != null) {
            ...
            output.append(s"[Dry Run] ${dir.getAbsolutePath}:\n")
            output.append(s"  Total items: ${items.length}\n")
            output.append(s"  Would DELETE: ${oldItems.length} items (not modified for $daysThreshold+ days) - ${humanReadableByteCountSI(deleteSize)}\n")
            output.append(s"  Would KEEP: ${recentItems.length} items (recently modified) - ${humanReadableByteCountSI(keepSize)}\n")
          } else {
            output.append(s"[Dry Run] ${dir.getAbsolutePath}: empty or not accessible\n")
          }
        } else {
    ...
    println("=" * 60)
    println("=== SUMMARY ACROSS ALL NODES ===")
    println("=" * 60)
    println(s"Total nodes processed: ${uniqueResults.size}")
    println(s"Total would DELETE: $grandTotalDeleteCount items - ${humanReadableByteCountSI(grandTotalDeleteSize)}")
    println(s"Total would KEEP: $grandTotalKeepCount items - ${humanReadableByteCountSI(grandTotalKeepSize)}")
    println("=" * 60)
